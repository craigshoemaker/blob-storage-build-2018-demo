...
const deleteBlob = (blobName, prefix) => {
...
$('#uploadFile').on('change', () => {
  ...
  if(isFileFormatSupported(prefix)){
    ...
    blobService.createBlockBlobFromBrowserFile(config.container, prefix + file.name, file, null, (error, result, response) => {
      if (error) {
        alert(error)
      } else {
...
const resultTemplate = $('#resultTemplate').html()
